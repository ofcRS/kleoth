...
public enum AppStyle: String, Sendable, CaseIterable {
    case compose, chat   // `terminal` removed 2026-09-07 — §10.3 item 15
    public static func classify(bundleId: String?) -> AppStyle   // case-insensitive; messengers → .chat, everything else (terminals, editors, AI chats, notes, mail, browsers, nil/"") → .compose
...
`AppStyle` tables (case-insensitive match on the lowercased bundle id) — **superseded 2026-09-03 by the compose/chat/terminal intensity model (§10.3 item 8), then reduced to compose/chat on 2026-09-07 (§10.3 item 15 — terminals are `.compose`); the original four-way `code/chat/prose/neutral` split is kept below for history:**
- ~~`.terminal` exact: `com.apple.terminal`, `com.googlecode.iterm2`, `dev.warp.warp-stable`, `net.kovidgoyal.kitty`, `io.alacritty`, `com.mitchellh.ghostty`~~ (removed 2026-09-07 — these are `.compose`)
...
- ~~terminal: plain text, no Markdown, one line unless the speaker enumerated; keep the speaker’s words (never turn a description into a command).~~ (removed 2026-09-07)
...
| Polish timeout (30 s ceiling — was 8 s, see §10.3 item 13) / HTTP error / 404'd model / bad JSON / truncated / hallucination guard | `DictationPolisher` | raw text pasted; `.warning(reason)` 3 s | ✓ `used_raw_fallback`, `fallback_reason` |
| Esc while `.polishing` | controller (`handleEscape` cancels only `polishTask`) | raw text pasted at once, no warning, pill runs on to `.done` | row logged as skipped (`polish_model: null`, `used_raw_fallback: false`) |
...
14. Same enumerated utterance into Terminal / Slack / Mail → Terminal and Mail restructure alike (numbered list), Slack stays light (§10.3 item 15 removed the terminal mode).
...
17. Wi-Fi off mid-polish → raw pasted within ~30 s (or at once on Esc). Wi-Fi off before Scribe → error within ~25 s.
...
5. **Default model switched to `z-ai/glm-5.3-flash` (2026-09-03, post-T8 fixer pass).** The T8 probes showed the design's `google/gemini-3.8-flash` default 404s on this OpenRouter account (`zdr-violation-by-account` — the Zero-Data-Retention guardrail excludes every `google/*` endpoint; the `json_object` fallback does not help), which made every dictation take the raw fallback and every summary fail. `ModelCatalog.defaultModel` and `DictationDefaults.polishModel` are now both `z-ai/glm-5.3-flash` (verified live: 200 with strict `json_schema` + `temperature`, Russian preserved, fillers removed); `google/gemini-3.8-flash` stays in `curatedFallback` (selectable, not default) and `retiredModels` still map the two dead slugs → the default. §1.1/§3.1/§3.12/§6.1 above describe the Gemini default as designed; this entry supersedes them. Because GLM 5.3 flash is a reasoning model (OpenRouter: "Reasoning is mandatory for this endpoint"), the polish call now sends `reasoning: {effort: "low"}` via a new optional `OpenRouterClient.complete(reasoning:)` argument, gated on `DictationDefaults.reasoningCappedModels` — measured: 0 reasoning tokens and 1.5–4.9 s (mean 3.2 s) vs 104–362 tokens and 4.8–14.2 s (mean ~8 s, one run past the 8 s budget) without it, identical output. The gate exists because the same parameter 404s on `meta-llama/llama-3.3-70b-instruct` under `require_parameters: true` and switches reasoning ON for `deepseek/deepseek-v4-flash`. `polishTimeout` stays 8 s. The Summarizer body is unchanged (test-pinned). **Correction (root fixer pass, same day):** the ZDR guardrail does NOT exclude every `google/*` endpoint. Re-probed live: on `google/gemini-3.8-flash` a body with `temperature` under `require_parameters: true` 404s with `json_schema` AND `json_object` alike (it forces routing onto the one excluded AI-Studio endpoint), while the same body WITHOUT `temperature` returns 200; `gemini-3.5-flash` and `gemini-3.1-pro-preview` return 200 with schema + temperature. The default switch stands (3.8-flash would still pay a doomed first attempt on every polish), but §9.34's "polish with `json_schema` + `temperature` on `gemini-3.8-flash` returns 200" does not hold on this account.
6. **Relaxed fallback retry in `OpenRouterClient.complete` (2026-09-03, root fixer pass).** The 400/404 recovery used to swap only the response format and re-send `temperature`/`reasoning`, so a parameter-routing 404 (item 5's `google/gemini-3.8-flash` + `temperature`, `meta-llama/llama-3.3-70b-instruct` + `reasoning`) failed identically on the retry and every dictation on those models pasted raw. It now retries once with `.jsonObject` (when the first attempt was `.jsonSchema`), `temperature: nil` and `reasoning: nil`, and also fires when the first attempt was already `.jsonObject`/`.none` but carried temperature or reasoning (skipped when the retry body would be identical). One relaxed retry, not a ladder — worst case stays two round trips inside the 8 s budget. `DictationDefaults.reasoningCappedModels` is thereby a latency optimization, not the only thing standing between a model and a 404. Also in this pass: `DictationPolishResult.raw` gained `cost` so the truncation / undecodable / translation-guard fallbacks (decided after a billed response) log what OpenRouter charged instead of `polish_cost: 0`, and `DictationPolisher.shortDescription(of:)` reduces an HTTP failure to its status (≤120 chars for anything else) before the reason reaches the pill / `fallback_reason`.
7. **Polish default → `google/gemini-3.5-flash-lite` + fallback model (2026-09-03, latency pass).** The user reported polishing felt slow; with ZDR relaxed, a polish-only benchmark through the real polisher (`dictate --text`) measured lite at 0.85–1.05 s median vs 3.6–4.1 s for the capped `z-ai/glm-5.3-flash`. `DictationPolisher` now tries `DictationDefaults.fallbackPolishModel` (`z-ai/glm-5.3-flash`) once when the primary fails with an HTTP error and ≥2 s of budget remain, so accounts that block Google still get polished text. `reasoningCaps` is per-model (the single allowlist in §3.7 is superseded). Supersedes item 5 for the dictation default; the summary default stays.
8. **Polish gate — short and chat dictations skip the model (2026-09-03, user request: "Scribe itself is doing pretty well… for casual texting the data returned by ElevenLabs is pretty much okay").** `PolishGate.decide(rawText:style:alwaysPolish:)` (KleothCore, tested) runs in `DictationController.run()` right after the empty-transcript check: `AppStyle.chat` targets skip at any length, every other target skips under `DictationDefaults.minimumWordsToPolish` (24 whitespace-separated words ≈ 10 s of speech; Cyrillic and Latin count alike). A skip is NOT a fallback: `DictationPolishResult.skipped(text:reason:)` (never produced by `DictationPolisher`) has `usedRawFallback == false`, no warning on the pill, `polish_model: null` and `polish_cost: 0` on the row, and the phase goes `transcribing → inserting` with no polishing wave. The Dictations detail pane shows an "As heard" badge whose tooltip recomputes the reason from the stored `app_bundle_id` + `raw_text` (no new log key). `Settings.dictationPolishAlways` (Keychain `dictation_polish_always`, strict "true", default off; Settings → Dictation toggle "Also clean up short dictations and chat messages") restores §5.10's always-polish flow. The `dictate` probe prints the gate's verdict but polishes anyway (it exists to time the polisher). Trade-off accepted: a short compose-mode utterance with a spoken self-correction ("Anna, sorry, Boris") now pastes both names; the dictionary still applies (Scribe keyterms), and the language is never touched.
9. **Pill motion rebuilt with a filmstrip loop (2026-09-03, after 0.2.0).** The user could not describe the animation and the agent could not see it, so the pill moved into the `KleothPillUI` library and gained `pillsandbox`: a control window for humans and a headless `--film` mode that captures the panel via `CGWindowListCreateImage` (own window, no permission) into PNG frames + a contact sheet the agent reads. The film showed the rise popping to the full listening shape while tucked (content-driven width does not animate) and a 6% squash. Now: explicit animated `capsuleSize`, a `MotionBeat`-driven keyframed squash-and-stretch along the travel axis, content reveal keyed to the beat, mic-level breathing, and a hover peek (`.activeAlways` tracking area; SwiftUI `.onHover` is inert while another app is active) that pulls the resting pill out with a mic glyph. Click-to-start from the peeking pill is deferred.
10. **Responsiveness (2026-09-03, after item 9).** The user read the 0.30 s hold gate plus the 0.55 s rise as lag and asked whether it was macOS. It was not: `minHold` is now 0.20 s, the pill gets a `.armed` state shown at key-down (the resting sliver hops out with the mic glyph on a 0.22 s peek spring; `.listening` grows out of it in place at `minHold`; a too-short tap keeps it out for the double-tap window, then sinks it), and every spring/keyframe was shortened to ~0.2–0.35 s. §3's "no UI at `.armed`" is therefore superseded: the armed pill carries no words and no bars, but it is visible. If the press still feels slow, the fallback the user named is dropping the double-tap so `.armed` can become `.listening` outright.
11. **External microphones (2026-09-06).** A Bluetooth headset mic (16 kHz mono) broke both captures: the AAC encoder caps at 48 kbps there and rejected the fixed 64/128 kbps request (`AudioFormat.aacSettings` now clamps to the encoder's applicable range), and the headset's A2DP→HFP switch ~0.1 s after start posted a configuration change that §7 told the capture to treat as a mid-utterance interruption — cutting every cold dictation to 0.1 s. §7's "restarting on the new device is out of scope" is superseded: `DictationCapture` and `MicCapture` reinstall the tap at the node's new format and restart, converting into the open file via `TapWriter`; `interrupted` now fires only when that restart fails.
12. **Crash after starting a dictation — root-caused from five crash reports (2026-09-06).** All five (`Kleoth-2026-09-04-212852` aside — a layout exception) died in `swift_task_isCurrentExecutor` → `swift_getObjectType` on a garbage pointer, twice inside `DictationHotkeyMonitor.start()`'s event closure and once inside `MeetingAudioPlayer`'s timer. The system log a few seconds before each shows AVFoundation raising `com.apple.coreaudio.avfaudio: Failed to create tap due to format mismatch, <1 ch, 48000 Hz>` from `DictationCapture.installTap` ← `start()` ← `handleArmed()`, with "Format mismatch: input hw 44100 Hz, client format 48000 Hz". AppKit's run loop catches the `NSException` (`HIExceptions FAULT`), but the unwind skips the Swift runtime's C++ destructors, so the thread-local `ExecutorTrackingInfo` keeps pointing at a popped stack frame and the next dynamic main-actor check reads garbage. Reproduced with an aggregate device at 44.1 kHz and then with the real headset: after the default input changes while the engine is idle, `inputNode.inputFormat(forBus:)` follows the hardware but `outputFormat(forBus:)` keeps the previous run's rate **for good** (`prepare()`/`reset()` do not refresh it; a tap at `outputFormat` raises every time, at `inputFormat` succeeds and the output format re-syncs on the next start). Two fixes, both landed: (a) `DictationCapture`, `MicCapture` and `MicrophoneSource` read `inputFormat(forBus: 0)` in `start()` and in their configuration-change handlers; (b) new `KleothObjC` target (`KLCatchObjCException`) + `catchingObjCExceptions(_:)` in KleothCapture wrap every `installTap`, so a refusal is a thrown `ObjCExceptionError` → `DictationCaptureError.engineFailed` → a `.failed` pill (and `log.error`), never a swallowed exception. Verified with a probe target driving the real `DictationCapture` through idle switches built-in 48 kHz ↔ WH-1000XM5 16 kHz (the crash sequence) — every start succeeds. Known leftover: a default-input switch DURING a session did not always post `AVAudioEngineConfigurationChange` in the probe (the engine reported running, delivered no more frames, `interrupted` stayed false) — the clip then ends at the switch without a warning; §7 "device switch mid-utterance" is therefore best-effort.
13. **Polish timeouts (2026-09-06).** 16 of 85 polishes on this Mac ended in "Polish timed out" at 47–178 words, yet the identical texts polish in 1.0–2.2 s through `dictate --text` on the shipped default. The day files showed why: every polished row recorded `polish_model: z-ai/glm-5.3-flash` — the default for a few hours on 09-03, persisted into the Keychain by `SettingsView.loadFromController` and never migrated when the default moved on (`ModelCatalog.retiredModels` only knows dead slugs). glm-5.3-flash + `reasoning low` measures 3.6–4.1 s median with a tail past 8 s. Changes: `DictationDefaults.retiredPolishModels` (`glm-5.3-flash` → `polishModel`) + `migratingPolishModel(_:)` (chains `ModelCatalog.migrating`), applied in `AppConfig.mergeSettingsFromKeychain`, `DictationController.setDictationModel` and persisted by `SettingsView.loadFromController`; the Settings picker no longer offers a retired polish slug (glm stays the automatic `fallbackPolishModel`). `polishTimeout` 8 → **30 s** — a hung-connection ceiling, not the expected wait (the user: a long dictation must never lose its polish to a timer). **Esc while `.polishing` cancels only the model call** (`polishTask`, an unstructured child; `cancelPipeline()` cancels both) and pastes the raw transcript at once, logged as `.skipped` ("Cancelled with Esc — pasted as heard.") — no warning, `polish_model: null`; Esc while transcribing/inserting still cancels the dictation. New log key `polish_seconds` (wall-clock of the model call, nil when skipped) so a slow run can be told apart from a long text. §3's 8 s budget and §7's "Polish timeout (8 s)" row are superseded.
14. **Headset stuck in hands-free mode after a dictation (2026-09-07).** The user: after a dictation on the WH-1000XM5 the headphones "sometimes still hang in this microphone headset mode… until I reopen Kleoth". Root cause, probed on this Mac: an `AVAudioEngine` whose `inputNode` has been touched keeps the input AUHAL — and so the input device — open until the engine **object is deallocated**; `engine.stop()` does not release it, and on a Bluetooth headset an open input device pins the link in HFP (16 kHz mic, phone-quality A2DP-less output). A stopped engine held the output at 16 kHz for as long as it lived (even one that was never started — reading `inputFormat(forBus:)` is enough), and the output returned to 44.1 kHz within a second of the engine being freed. `DictationCapture` kept one engine for the controller's lifetime (= the app's), hence "until I reopen Kleoth"; the meeting `MicCapture` lived until its `Recorder` was released after the 2-channel combine. Fix: both captures build the engine in `start()` as a local (every failure path frees it) and release it in `stop`/`cancel`/the configuration-change give-up path (`DictationCapture.quiesce()`, `MicCapture.releaseEngine()`). Cost: a fresh engine takes ~200 ms to reach `engine.start()` on the headset vs ~55 ms warm (measured 3×3), so `DictationController.handleArmed()` now shows the `.armed` pill BEFORE `capture.start()` (a failure replaces it with `.failed`). Verified with a throwaway probe target driving the real classes: dictation start/stop, an 80 ms armed tap cancelled, a second session on the same instance and a `MicCapture` start/stop all return the headset to 44.1 kHz while the capture objects stay alive; clips still 1.92 s of a 2 s session (the HFP restart path intact). `MicrophoneSource` (screen recording) is untouched — its owner releases it at stop. §5.10's "own AVAudioEngine" wording now means one per session.
